...
extern "C"
{
    ...
    { return new Adwaita::StyleConfig(parent); }
}
...
namespace Adwaita
{
    ...
    void StyleConfig::save( void )
    {
        ...
        QScopedPointer<KConfig> kde4Config(StyleConfigData::self()->config()->copyTo(kde4ConfigDirPath+"/adwaitarc", 0));
        kde4Config->sync();
        #endif

        // emit dbus signal
        QDBusMessage message( QDBusMessage::createSignal( QStringLiteral( "/AdwaitaStyle" ),  QStringLiteral( "org.kde.Adwaita.Style" ), QStringLiteral( "reparseConfiguration" ) ) );
